...
export class LiveScheduleComponent implements OnInit {
  ...
  intializeNextPlan() {
    ...
    var gapDay = this.getGapBetweenDate(this.now, scheduleDate);
    if (gapDay > 0) {
      this.nextPlan = planArrays[0];
      this.nextPlanTime = this.getDateAddedTime(scheduleDate, planArrays[0].time);
    } else {
      //다음 계획을 현재 시각과 대조하여 찾는다
      planArrays.forEach((el, idx) => {
        // 시간과 분을 분리
        scheduleDate = this.getDateAddedTime(scheduleDate, el.time);
  ...
  */
  ...
  private setDayHourMin(from: Date, to: Date) {
    var timecalculator = new TimeCalculator(from, to);

    this.leftDay = timecalculator.getDay();
    this.leftHour = timecalculator.getHour(this.leftDay);
    this.leftMin = timecalculator.getMin(this.leftDay, this.leftHour);
  }
  ...
  getGapBetweenDate(from: Date, to: Date) {
    var timecalculator = new TimeCalculator(from, to);
    return timecalculator.getDay();
  }

  getDateAddedTime(source: Date, planTime: String): Date {
    var times = planTime.split(":");
    source.setHours(Number(times[0]));
